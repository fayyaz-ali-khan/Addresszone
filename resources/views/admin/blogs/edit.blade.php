...
@section('main-content')
    <x-Admin.Shared.form-page-container :title="request('tab')=='comments' ? 'Blog Details' : 'Edit Blog'">
        <form id="email-template-form" action="{{ route('admin.blogs.update', $blog->id) }}"
                    ...
                    <div class="form-group">
                        ...
                        <select id="category" name="category" class="form-control mb-3">
                            ...
                            @foreach ($categories as $category)
                                <option
                                    value="{{ $category->id }}" @selected(old('category', $blog->blog_category_id) == $category->id)>{{ $category->name }}
                                </option>
                            @endforeach
            ...
            <span class="d-flex justify-content-between w-100">
                @if(request('tab')=='comments')
                    <a href="{{ route('admin.blogs.index') }}" class="btn btn-primary mt-2">Ok</a>
                @else
...
@push('js')
    <script>
        $(document).ready(function () {
            ...
            @if(request('tab')=='comments')
                // Make text-based fields readonly
                $('input[type="text"], input[type="email"], input[type="number"], input[type="password"], textarea').attr('readonly', true);

                // Disable selects, checkboxes, radios, file inputs, buttons
                $('select, input[type="checkbox"], input[type="radio"], input[type="file"], button').attr('disabled', true);
            @endif
