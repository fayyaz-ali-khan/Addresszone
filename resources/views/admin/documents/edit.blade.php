                ...
                if (userId) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            const user_documents = response.documents ?? [];

                            if (user_documents.length > 0) {

                                let docs = '';
                                for (let index = 0; index < user_documents.length; index++) {
                                    const element = user_documents[index];
                                    docs += `
                                        <tr>
                                            <td>${element.document_title}</td>
                                            <td><a href="${element.document_file}" download>Download</a></td>
                                        </tr>
                                    `;
                                }
                                $("#document-list").html(docs);


                            } else {
                                $("#document-list").html("<p>No documents exists</p>");
                            }
